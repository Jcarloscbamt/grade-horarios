            ...
            <div class="row g-2 align-items-center">
                <div class="col-md-8">
                    <div class="input-group">
                        ...
                        @if($search)
                        <button class="btn btn-outline-secondary" wire:click="$set('search', '')" title="Limpar"><i class="bi bi-x-lg"></i></button>
                        @endif
                    </div>
                </div>
                <div class="col-md-4">
            ...
            <div class="table-responsive" style="max-height:calc(100vh - 220px);overflow-y:auto">
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ !$curso->ativo ? 'opacity-50' : '' }}">
                            <td class="ps-3 fw-medium">{{ $curso->nome }}</td>
                            <td>
                                <span class="badge fw-semibold" style="background:{{ $curso->cor_grade ?? '#E30613' }};color:white">
                                    {{ $curso->sigla }}
                                </span>
                            </td>
                            <td>{{ $curso->nivel }}</td>
                            ...
                            <td>
                                ...
                                @if($curso->telefone_coord)<small class="text-muted">{{ $curso->telefone_coord }}</small>@endif
                            </td>
                            <td class="text-center">
                                <div class="d-flex align-items-center justify-content-center gap-2">
                                    <div class="rounded-circle border" style="width:24px;height:24px;background:{{ $curso->cor_grade ?? '#E30613' }}"></div>
                                    <code style="font-size:11px">{{ $curso->cor_grade ?? '#E30613' }}</code>
                                </div>
                            </td>
                            <td class="text-center">
                                @hasanyrole('admin|coordenador')
                                <button wire:click="toggleAtivo({{ $curso->id }})"
                                    class="btn btn-sm {{ $curso->ativo ? 'btn-success' : 'btn-secondary' }}"
                                    title="{{ $curso->ativo ? 'Desativar' : 'Ativar' }}">
                                    <i class="bi bi-circle-fill me-1" style="font-size:8px"></i>
                                    {{ $curso->ativo ? 'Ativo' : 'Inativo' }}
                                </button>
                                @else
                                <span class="badge {{ $curso->ativo ? 'bg-success' : 'bg-secondary' }}">
                                    {{ $curso->ativo ? 'Ativo' : 'Inativo' }}
                                </span>
                                @endhasanyrole
                            </td>
                            <td class="text-center pe-3">
                                @hasanyrole('admin|coordenador')
                                <button wire:click="edit({{ $curso->id }})" class="btn btn-sm btn-outline-secondary me-1" title="Editar"><i class="bi bi-pencil"></i></button>
                                @endhasanyrole
                                @hasrole('admin')
                                <button wire:click="confirmDelete({{ $curso->id }})" class="btn btn-sm btn-outline-danger" title="Excluir"><i class="bi bi-trash"></i></button>
                                @endhasrole
                            </td>
                        </tr>
                        @empty
                        <tr><td colspan="8" class="text-center text-muted py-5"><i class="bi bi-inbox fs-3 d-block mb-2"></i>Nenhum curso encontrado.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" wire:model="telefone_coord" class="form-control @error('telefone_coord') is-invalid @enderror" placeholder="(65) 3612-9966" maxlength="15">
                        ...
                        <div class="col-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" wire:model="ativo" id="ativoCurso">
                                <label class="form-check-label fw-medium" for="ativoCurso">
                                    Curso <strong>{{ $ativo ? 'Ativo' : 'Inativo' }}</strong>
                                    <span class="badge ms-1 {{ $ativo ? 'bg-success' : 'bg-secondary' }}">
                                        {{ $ativo ? 'Ativo' : 'Inativo' }}
                                    </span>
                                </label>
                            </div>
                        ...
                        <div class="col-12">
                            <hr class="my-1">
                            <label class="form-label fw-medium"><i class="bi bi-palette me-1"></i>Cor da Grade de Horários <span class="text-danger">*</span></label>
                            <div class="d-flex flex-wrap gap-2 mb-2">
                                @foreach($coresSugeridas as $hex => $nome)
                                <button type="button" wire:click="$set('cor_grade', '{{ $hex }}')" title="{{ $nome }}"
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <input type="color" wire:model.live="cor_grade" class="form-control form-control-color" style="width:48px;height:38px;padding:2px;cursor:pointer">
                                <input type="text" wire:model.live="cor_grade" class="form-control @error('cor_grade') is-invalid @enderror" placeholder="#E30613" maxlength="7" style="font-family:monospace;max-width:120px">
                                <div class="d-flex align-items-center gap-2 ms-2 p-2 rounded flex-grow-1" style="background:{{ $cor_grade }};min-height:38px">
                                    <span class="fw-bold text-white" style="font-size:13px;text-shadow:0 1px 2px rgba(0,0,0,0.5)">{{ strtoupper($sigla ?: 'ADS') }} — Preview</span>
                                </div>
                            </div>
