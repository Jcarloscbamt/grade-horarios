...
<div>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold mb-0">Disciplinas</h2>
            <small class="text-muted">Gerenciamento de disciplinas dos cursos</small>
        </div>
    ...
    <div class="card border-0 shadow-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge fw-semibold"
                                      style="background:{{ $disciplina->curso->cor_grade ?? '#6c757d' }};color:white">
                                    {{ $disciplina->curso->sigla ?? '—' }}
                                </span>
                                <span class="ms-1 small">{{ $disciplina->curso->nome ?? '—' }}</span>
                            </td>
                            <td class="text-center">
                                <span class="badge bg-secondary">{{ $disciplina->semestre_grade }}º sem</span>
                            </td>
                            ...
                            <td class="text-center pe-3">
                                @hasanyrole('admin|coordenador')
                                <button wire:click="edit({{ $disciplina->id }})"
                                    class="btn btn-sm btn-outline-secondary me-1" title="Editar">
                                    <i class="bi bi-pencil"></i>
                                </button>
                                @endhasanyrole
                                @hasrole('admin')
                                <button wire:click="confirmDelete({{ $disciplina->id }})"
                        ...
                        <tr>
                            <td colspan="6" class="text-center text-muted py-5">
                                <i class="bi bi-inbox fs-3 d-block mb-2"></i>Nenhuma disciplina encontrada.
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="card-footer bg-white border-top-0">{{ $disciplinas->links() }}</div>
        @endif
    </div>
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select wire:model="semestre_grade"
                                class="form-select @error('semestre_grade') is-invalid @enderror">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" wire:model="carga_horaria"
                                class="form-control @error('carga_horaria') is-invalid @enderror"
                                placeholder="Ex: 80" min="1" max="999">
                            @error('carga_horaria') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Separador: Sala Preferencial --}}
                        <div class="col-12">
                            <hr class="my-1">
                            <label class="form-label fw-medium">
                                <i class="bi bi-building me-1 text-muted"></i>
                                Sala Preferencial para Alocação Automática
                            </label>
                            <div class="text-muted small mb-2">
                                O gerador de grade usará este tipo + bloco para alocar a sala automaticamente.
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select wire:model.live="tipo_sala"
                                class="form-select @error('tipo_sala') is-invalid @enderror">
                        ...
                        <div class="col-md-6">
                            ...
                            <select wire:model="bloco_preferencial"
                                class="form-select @error('bloco_preferencial') is-invalid @enderror"
                                {{ !$tipo_sala ? 'disabled' : '' }}>
                                <option value="">Qualquer bloco</option>
                                @foreach($blocos as $bloco)
                                <option value="{{ $bloco }}">Bloco {{ $bloco }}</option>
                                @endforeach
                            </select>
                            @error('bloco_preferencial') <div class="invalid-feedback">{{ $message }}</div> @enderror
                            @if(!$tipo_sala)
                            <div class="form-text text-muted">Selecione um tipo de sala primeiro.</div>
                            @endif
                        </div>
                            ...
                            <div class="p-2 rounded border bg-light d-inline-flex align-items-center gap-2">
                                ...
                                <span class="fw-semibold" style="font-size:13px">
                                    ...
                                    <span class="text-primary">
                                        {{ $tipo_sala }}{{ $bloco_preferencial ? ' — Bloco ' . $bloco_preferencial : ' (qualquer bloco)' }}
                                    </span>
                                </span>
                            </div>
                        </div>
            ...
            <div class="modal-content border-0 shadow">
                <div class="modal-body text-center py-4">
                    ...
                    <p class="text-muted mb-0">
                        Tem certeza que deseja excluir esta disciplina?<br>
