...
<div>
    ...
    {{-- Modal Incluir/Editar (modal-xl com disciplinas e disponibilidade) --}}
    @if($showModal)
            ...
            <div class="modal-content border-0 shadow">
                ...
                <div class="modal-body p-0">
                    <div class="row g-0">
                        ...
                        {{-- Coluna Esquerda — Dados + Disponibilidade + Status --}}
                        <div class="col-lg-5 border-end p-4">
                            ...
                            {{-- Botão Salvar na coluna esquerda --}}
                            <div class="d-flex gap-2 mt-3 mb-1">
                                <button type="button" class="btn btn-primary flex-grow-1" wire:click="save" wire:loading.attr="disabled">
                            ...
                            <div class="text-muted small mt-2"><i class="bi bi-info-circle me-1"></i>Selecione ao menos um dia antes de vincular disciplinas.</div>
                            @endif
                        </div>
                        ...
                        <div class="col-lg-7 p-4 d-flex flex-column" style="background:#f8f9fa">
                            ...
                            {{-- LISTA DE VÍNCULOS (mostrada primeiro para edição) --}}
                            @if(count($vinculos) > 0)
                            <div class="mb-3" style="max-height:220px;overflow-y:auto">
                                        ...
                                        <div class="card-body py-2 px-3">
                                            <div class="d-flex justify-content-between align-items-start">
                                                ...
                                                <div class="d-flex gap-1 ms-2">
                                                    <button type="button" wire:click="editarVinculo({{ $i }})" class="btn btn-sm btn-outline-secondary" title="Editar vínculo"><i class="bi bi-pencil"></i></button>
                                                    <button type="button" wire:click="removerVinculo({{ $i }})" class="btn btn-sm btn-outline-danger" title="Remover vínculo"><i class="bi bi-trash"></i></button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="text-muted text-end mt-1" style="font-size:11px">
                                    <i class="bi bi-info-circle me-1"></i>{{ count($vinculos) }} vínculo(s)
                                </div>
                            </div>
                            @endif

                            <div class="card border-0 shadow-sm mb-2" style="{{ $editandoVinculoIdx >= 0 ? 'border:2px solid #f59e0b !important' : '' }}">
                                ...
                                <div class="card-body p-3">
                                    ...
                                    <div class="border rounded bg-white" style="max-height:180px;overflow-y:auto">
                                        ...
                                        <button type="button" wire:click="selecionarDisciplina({{ $disc['id'] }}, '{{ addslashes($disc['nome']) }}', '{{ addslashes($disc['curso_nome']) }}', {{ $disc['curso_id'] }})"
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                @if($disc['semestre_grade'])<span class="badge bg-secondary rounded-pill ms-2" style="font-size:10px">{{ $disc['semestre_grade'] }}º sem</span>@endif
                                            </div>
                                        </button>
                                        @empty
                                    ...
                                    <div class="text-muted small mt-1"><i class="bi bi-info-circle me-1"></i>Digite mais um caractere para buscar...</div>
                                    @endif
                                    @else
                                    <div class="d-flex align-items-center justify-content-between rounded p-2 mb-3 border border-primary" style="background:#e8f0fe">
                                        <div>
                                        ...
                                        <button type="button" wire:click="cancelarSelecao" class="btn btn-sm btn-outline-secondary ms-2"><i class="bi bi-arrow-left me-1"></i>Trocar</button>
                                    </div>
                                    <div class="mb-2">
                                        ...
                                        @if(count($turmasDoVinculo) === 0)<div class="text-warning small mt-1"><i class="bi bi-exclamation-triangle me-1"></i>Nenhuma turma cadastrada para este curso.</div>@endif
                                    </div>
                                    @if($sel_turma_id)
                                    <div class="mb-3">
                                        ...
                                        @if(empty($disponibilidade))
                                        <div class="alert alert-warning py-2" style="font-size:12px"><i class="bi bi-exclamation-triangle me-1"></i>Defina a disponibilidade geral do professor primeiro (coluna esquerda).</div>
                                        @else
                                        <div class="d-flex gap-2 flex-wrap">
                                            @foreach($diasNomes as $num => $label)
                                            @php $disponivel = in_array($num, $disponibilidade); $marcado = in_array($num, $sel_dias); @endphp
                                            <div class="form-check form-check-inline m-0" title="{{ !$disponivel ? 'Dia fora da disponibilidade geral' : '' }}">
                                                ...
                                                <label class="form-check-label fw-semibold" for="sdia_{{ $num }}">
                                                    <span class="badge {{ !$disponivel ? 'bg-light text-muted border' : ($marcado ? 'bg-primary' : 'bg-light text-dark border') }}"
                                                          style="font-size:12px;min-width:38px;cursor:{{ $disponivel ? 'pointer' : 'not-allowed' }};opacity:{{ $disponivel ? '1' : '0.4' }}">
                                                        {{ $label }}
                                                    </span>
                                                </label>
                                            </div>
                                        ...
                                        <div class="text-muted mt-1" style="font-size:11px"><i class="bi bi-info-circle me-1"></i>Apenas os dias da disponibilidade geral estão habilitados.</div>
                                        @endif
                                    </div>
                                    @endif
                            ...
                            <div class="text-center text-muted py-3">
                                <i class="bi bi-book fs-2 d-block mb-1 opacity-25"></i>
                                <small>Nenhuma disciplina vinculada.<br>Use o formulário acima para adicionar.</small>
                            </div>
                            @endif
                        </div>
                    </div>
